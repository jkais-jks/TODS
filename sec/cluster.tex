...
In Eq.~\ref{eqa:cluster-1}, given these cluster, we can first rewrite the sum of N feature distances (i.e., $\min_{c_j\in \core_k}\lVert\mathbf{x}_i - \mathbf{x}_{\gamma(j)}\rVert$) to the sum of \groups summations, each of which is the sum of $|\group_u|$ such distances. Second, for each cluster, the sum can be bounded by the cluster size multiplying the maximum distance ($\max_{v \in \group_u} \min_{c_j\in \core_k}\lVert\mathbf{x}_v - \mathbf{x}_{\gamma(j)}\rVert$) in the cluster. However, computing this bound is also expensive. To overcome this, third, we use the max-min inequality ~\ref{} to simplify calculations. 

In this way, instead of iterating $\trainc$, we can iterate the much smaller $\group$ to derive the maximum feature distance(i.e., $s_{\gamma(j)v} = \Vert\mathbf{x}_v - \mathbf{x}_{\gamma(j)} \Vert, j \in [1,|\core_k|]$) between each $c_j \in \core_k$ and tuples in each cluster $\group_u$. Then we can assign the cluster $\group_u$ to the tuple with the minumum distance, i.e., $\min_{c_j\in C_k} s_{\gamma(j)v}$.
