...
Next, suppose that there are $y+1$ tuples ($y$ tuples in $G_u$ and one tuple in $\hat{C}$) with missing values, and thus $l^{y+1}$ possible worlds are generated for $G_u \cup \hat{C}$. To achieve further acceleration, we can select top-$l_g$ (\eg 3) possible worlds from the $l^y$ ones to compute the expectation.
...
\jks{\noindent \textbf{Information entropy}The information entropy~\cite{entropy} is calculated based on the probability distribution of events to describe the uncertainty or amount of information associated with a random variable, as follows:}
