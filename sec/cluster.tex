...
% Given a tuple $t_j\in \hat{C}$ and cluster $\group_u \in \mathcal{G}$, we first brute all possible worlds of $t_v$ and compute the probability(\ie \{$q_1, q_2, ..., q_k$\}) of each possible world(\ie \{$\world_1, \world_2, ..., \world_k$\}). For each possible world $\world_i$, we use $t_{j}$ denote the tuple and then we can get a tuple-group distance(\ie $s_{jv}$). Then we can use $s_{jv} = \sum_{i=1}^k q_i s_{jv}$ to denote the expectation of tuple-group distance. Formally, following Eq. ~\ref{eqa:cluster-2}, we have:
